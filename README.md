...
# The (Im)Possibility of Inexpensively Improving the Data
In an ideal world we would gather more data for both the classification and forecasting routines in order to improve accuracy of both models. For the case of improving the classification accuracy, we need to wait many more years to gather more data. As I mentioned previously, with the suggestion that Café Casa Martinez follows the zoca schedule, they will reach an equilibrium where . With regard to improving the weather forecasting, more data may increase accuracy, however the rainfall totals recorded by Café Casa Martinez represent the best available data for a few reasons. First, the nearest weather station to the farm is at a much lower altitude with a drier climate. Scraping data from the web or purchasing access to a weather API would not be helpful in this case because the available weather stations do not record the true weather conditions at high elevation where the farm is. At any rate, the published rainfall data online in the region surrounding Finca Tulipan does not extend to as early of a year as the rainfall totals recorded by Café Casa Martinez. The data they presented me represents the best data in existence, barring the possibility that a neighboring farm rigorously recorded rainfall data for longer.
