...
## Goals
[Café Casa Martinez](https://www.cafecasamartinez.com/) is an organic coffee company with a farm called Finca Tulipan in the Santander department of Colombia. The goals of this consulting project are to:
...
## Summary of Results
1. Based on my analysis of the data, it's clear that regular upkeep of the fields - also known as renewal or zoca - has a stronger correlation with total yearly production than rainfall does. To help increase profits while minimizing short-term disruptions, I propose a renewal schedule for the farm that takes into account various factors. This schedule will recommend which fields should be renewed each year, and farmers can access the recommendations through this [dashboard](https://cafe-casa-martinez-app.herokuapp.com/).
2. My analysis shows that rainfall has a statistically significant positive correlation with total yearly production during the month of May, but not during any other month of the year. Therefore, it is especially important  to water crops during May if there isn't enough rainfall. While irrigating crops year-round would be too expensive for the farm, there is an affordable solution: the farmers can use rainwater collected in cisterns already located on the farm to water crops during dry spells in May. This way, they can ensure that their crops get the water they need to thrive without incurring significant extra costs.
3. The data shows that there has been no significant change in rainfall totals over time. Instead, the data displays a mean-reverting pattern with no discernable statistical trend towards either an increase or decrease in rainfall. This means that the farm can expect rainfall to remain relatively consistent in the future, allowing for more accurate long-term planning of irrigation and other crop management practices.
...
## Products 
Café Casa Martinez now has access to several key products that can help them optimize their coffee production and maximize their profits. These include:
1.	A yearly timeline: We provided the farm with a detailed timeline of the steps they should take each year to predict and maximize their yearly profit. This timeline takes into account factors like rainfall and production costs, and provides clear guidance on the actions that the farmers should take throughout the year to achieve their goals.
2.	An interactive [dashboard](https://cafe-casa-martinez-app.herokuapp.com/): I developed an interactive dashboard using Plotly, Dash, and Heroku, which provides farmers with a user-friendly interface to access key data related to production, profitability, and rainfall on their farm. This dashboard enables the farmers to make data-driven decisions about how to manage their crops every year.
## Coffee Background
Café Casa Martinez cultivates three varieties of coffee: Castillo, Tabi, and Cenicafe 1. These varieties grow in the shade of trees and only produce fruit once a year in this region of Colombia. The plants flower between February and April and yield fully ripened fruit between October and December, from which the coffee beans are extracted.

Coffee plants do not produce the same quantity of beans every year, even with consistent rainfall. The amount of fruit produced by each plant depends on the number of years since it was planted. The production curve is as follows. In the year of sowing and the following year, no fruit is produced. In the third year, the plant produces fruit at 50% of its maximum. In the fourth year, maximum fruit production is achieved and in subsequent years production gradually decreases. To maintain optimal production, coffee farmers perform a process called renewal, or "zoca," in the sixth year after planting. This involves cutting the plant near its base, allowing it to regrow from the stump and return to maximum production after two more years. A plant may be renewed up to five times. The following figure shows the production fraction as a function of year, covering the sowing cycle and three renewal cycles.

<img src="https://github.com/pdrew32/Cafe-Casa-Martinez/blob/master/figures/production_fraction_vs_year.png" width="400">

Farms like Finca Tulipan have multiple coffee fields that were sown and renewed in different years, meaning that in any given year, they have fields in all stages of their renewal cycles. To ensure consistent production, it is best to stagger sowing and renewal in different fields.

For this consulting project, Café Casa Martinez provided me with daily rainfall records dating back to October 2006, yearly coffee production totals since 2008, records of the month and day of sowing and renewal, and the number of plants in each of their 14 lots.
...
1. In January, use the LightGBM model to predict monthly rainfall for the entire year. Next, predict total production with the May and October rainfall estimates and predict whether the year will be profitable with May and October rainfall estimates. This is their first, coarsest estimate of production/profitability for the year.
...
3. In June, use the observed May rainfall totals to rerun a more accurate projection of the production for the year.
4. In November, use the observed May and October rainfall totals to run a more accurate projection of the profitability for the year.
