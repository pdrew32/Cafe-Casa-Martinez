...
This leads us to our second recommendation to maximize production. **If rainfall in the month of May is going to be below 203.2 mm, the farm should consider watering their lots.** But one never knows how much rain is going to fall in May ahead of time, so next I explore whether machine learning can accurately predict May rainfall totals. 

First, I investigate the maximum possible accuracy assuming we could perfectly predict May rainfall totals using machine learning. This will obviously not be achievable, but it is important to investigate given the scatter in the relation between May rainfall totals and total production. I adopt the value of the best fit relation at the true may rainfall total of each year for which we have data in the above figure and ask whether it is above the profit threshold. I then divide that by the true number of years that were above the profit threshold and find that if we had perfect accuracy with our machine learning prediction of May rainfall for years between 2008—2021 we would have been correct 80% of the time. This suggests we should not expect to be more than 80% accurate in our prediction of whether or not to irrigate.
