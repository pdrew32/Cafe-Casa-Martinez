...
The three red lines denote residuals where the adoption of the best fit value would result in an incorrect classification of profitable or unprofitable. In other words, for these three data points the predicted value on the best fit relation is on the other side of the profit threshold from the true production value. We will see that we have better predictive power if we recast this as a classification problem where years are classified in terms of profitable versus non profitable and then classification algorithms are used.
